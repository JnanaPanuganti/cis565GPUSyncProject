...
// *********************************************************************
const char* cSourceFile = "Mutex_FandA.cl";
...
int main(int argc, char **argv)
{
	...
	shrSetLogFileName ("Mutex_FandA.txt");
    ...
    lock_turn = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, sizeof(int), NULL, &ciErr1);
	lock_ticket = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, sizeof(int), NULL, &ciErr1);
	sleep_for = sleep_for = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, sizeof(int)*NUM_BLOCKS, NULL, &ciErr1);
    ...
    // Set the Argument values
   	...
   	ciErr1 = clSetKernelArg(ckKernel, 0, sizeof(cl_mem), (void*)&lock_turn);
	ciErr1 |= clSetKernelArg(ckKernel, 1, sizeof(cl_mem), (void*)&lock_ticket);
	//ciErr1 |= clSetKernelArg(ckKernel, 2, sizeof(cl_mem), (void*)&sleep_for);
   ...
   // ciErr1 |= clSetKernelArg(ckKernel, 1, sizeof(cl_int), (void*)&iNumElements);
	...
	ciErr1 = clEnqueueWriteBuffer(cqCommandQueue, lock_turn, CL_FALSE, 0, sizeof(int) ,&init_lock, 0, NULL, NULL);
    ciErr1 = clEnqueueWriteBuffer(cqCommandQueue, lock_ticket, CL_FALSE, 0, sizeof(int) ,&init_lock, 0, NULL, NULL);
...
void Cleanup (int argc, char **argv, int iExitCode)
{
    ...
    if(sum)clReleaseMemObject(sum);
	 if(lock_turn)clReleaseMemObject(lock_turn);
	 if(lock_ticket)clReleaseMemObject(lock_ticket);
